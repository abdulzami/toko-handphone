...
?>
			<ol class="breadcrumb bc-3">
				...
				<li class="active">
							...
							<a href="table-handphone.php"><strong>Data Pegawai</strong></a>
					</li>
					</ol><br>
<legend>
<button type="button" class="btn btn-success  btn-sm btn-icon icon-left" data-toggle="modal" data-target="#exampleModal"><i class="entypo-doc-text"></i>Add Data</button>
<h2>Data Pegawai</h2>
</legend>
<table class="table table-bordered datatable" id="table-1">
	...
	<?php
		$hasil = $db->QUERY("SELECT * FROM tabel_pegawai");
		...
		<td><!--<a href="edit_absensi.php?id=<?php echo $row['id'];?>" class="btn btn-info btn-sm btn-icon icon-left">
				...
				<a href="hapus-pegawai.php?id=<?php echo $row['id'];?>" class="btn btn-danger	 btn-sm btn-icon icon-left" onclick="return confirm('Apakah Anda yakin ingin menghapus ini?');">
					<i class="entypo-trash"></i>
					Hapus
				</a>
				<?php echo "<a href='#myModal' class='btn btn-info btn-sm btn-icon icon-left' id='custId'  data-toggle='modal' data-id=".$row['id']."><i class='entypo-reply'></i>Edit</a>"; ?>
		</td>								
		</tr>
		<?php
		$no++;
